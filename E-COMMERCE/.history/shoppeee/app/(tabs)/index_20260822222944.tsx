...
export default function HomeScreen() {
  ...
  // Simulate an initial fetch so the ActivityIndicator has a real purpose.
  useEffect(() => {
      ...
      {search.trim().length > 0 && searchSuggestions.length > 0 && (
        <View style={styles.suggestionsDropdown}>
          {searchSuggestions.map((product) => (
            ...
            >
              <Ionicons name="search-outline" size={16} color="#868E96" />
              <View style={styles.suggestionText}>
                <Text style={styles.suggestionName} numberOfLines={1}>
                  {(() => {
                    const query = search.trim();
                    const matchStart = product.name
                      .toLowerCase()
                      .indexOf(query.toLowerCase());

                    if (matchStart < 0 || !query) return product.name;

                    return (
                      <>
                        {product.name.slice(0, matchStart)}
                        <Text style={styles.suggestionHighlight}>
                          {product.name.slice(matchStart, matchStart + query.length)}
                        </Text>
                        {product.name.slice(matchStart + query.length)}
                      </>
                    );
                  })()}
                </Text>
