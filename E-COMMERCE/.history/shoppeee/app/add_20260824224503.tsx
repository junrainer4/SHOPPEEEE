...
// Converts Web Blobs/URIs to persistent compressed Data URIs
const processWebImage = (uri: string): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Limit size to max 250px so it fits safely in localStorage without memory limits
      const maxDim = 250;
      let width = img.width;
      ...
      // Returns a clean, permanent data URI
...
const App = () => {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      const finalUri = await processWebImage(rawUri);
      handleChange("image", finalUri);
    }
  };
