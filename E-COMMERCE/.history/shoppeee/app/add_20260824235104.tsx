...
import { Product, ProductFormErrors, ProductFormValues } from "../src/types";
...
const App = () => {
  ...
  const existingCategories = useMemo<string[]>(() => {
    const rawCategories: string[] = products
      .map((p: Product) => {
        const trimmed = p.category?.trim();
        if (!trimmed) return undefined;
        return trimmed.charAt(0).toUpperCase() + trimmed.slice(1).toLowerCase();
      })
      .filter((c: string | undefined): c is string => Boolean(c && c.length > 0));

    const uniqueCategories: string[] = Array.from(new Set(rawCategories));
    return uniqueCategories.sort((a: string, b: string) => a.localeCompare(b));
  }, [products]);

  const handleChange = (field: keyof ProductFormValues, value: string) => {
    setForm((prev: ProductFormValues) => ({ ...prev, [field]: value }));
  ...
  const handleSubmit = async () => {
    ...
    const formattedCategory =
      form.category.trim().charAt(0).toUpperCase() +
      form.category.trim().slice(1).toLowerCase();
      ...
      >
        ...
        <View style={styles.chipContainer}>
          {existingCategories.map((cat: string) => {
            const isSelected =
              !isCustomCategory && form.category.toLowerCase() === cat.toLowerCase();
            return (
              <Pressable
                key={cat}
                style={[
        ...
        {isCustomCategory && (
          <TextInput
            ...
            value={form.category}
            onChangeText={(t: string) => handleChange("category", t)}
