...
// Helper to resize and compress web images so they fit inside localStorage safely
const compressWebImage = (uri: string): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const maxDim = 300; // Constrain dimensions to 300x300 max
      let width = img.width;
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      if (asset.base64 && !asset.base64.startsWith("data:")) {
        rawUri = `data:image/jpeg;base64,${asset.base64}`;
      }

      const compressedUri = await compressWebImage(rawUri);
      handleChange("image", compressedUri);
    }
  };
