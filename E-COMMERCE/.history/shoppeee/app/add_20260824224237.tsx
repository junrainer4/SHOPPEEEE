...
const App = () => {
  ...
  const handlePickImage = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      allowsEditing: true,
      aspect: [1, 1],
      quality: 0.2, // Low quality to keep string size very small for localStorage
      base64: true,
    });
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      if (Platform.OS === "web") {
        // Fetch web blob and convert directly to Data Base64 URI so it never expires on refresh
        try {
          ...
          if (asset.base64) {
            handleChange("image", `data:image/jpeg;base64,${asset.base64}`);
          }
        }
      } else {
