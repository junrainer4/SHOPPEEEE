...
// Converts ANY web image format (AVIF, WEBP, PNG, etc.) to standard JPEG Data URI
const convertToStandardJpg = (file: File): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onerror = () => reject("Failed to parse image format");
...
const App = () => {
  ...
  const handlePickImage = async () => {
    if (Platform.OS === "web") {
      // Direct Web File Input to bypass Expo Web Blob limitations
      ...
      input.onchange = async (e: any) => {
        ...
        if (file) {
          try {
            const dataUrl = await convertToStandardJpg(file);
